...
import digital.dutton.example.coordinators.dataCoordinator.DataCoordinator
import digital.dutton.example.coordinators.dataCoordinator.updateSampleString
import digital.dutton.example.coordinators.languageCoordinator.LanguageCoordinator
import digital.dutton.example.coordinators.languageCoordinator.updateCurrentContent
import digital.dutton.example.coordinators.notificationCoordinator.NotificationCoordinator
import digital.dutton.example.coordinators.notificationCoordinator.sendSampleIntent
import digital.dutton.example.models.constants.DebuggingIdentifiers
import digital.dutton.example.models.keys.MainActivityStateKeys
...
import digital.dutton.example.utils.system.getOrientation
...
class MainActivity : ComponentActivity() {
      ...
      object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
          ...
          when (intent.action) {
            ...
            SystemNotifications.onLanguageContentUpdateIntent ->
                onLanguageContentUpdateIntent(
                    intent = intent,
                )
            SystemNotifications.onUpdateExperienceStateIntent ->
                onUpdateExperienceStateIntent(
                    intent = intent,
                )
          }
        }
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val orientation =
        when (getOrientation(context = baseContext)) {
    ...
    // If you were to use this within a composable that has custom actions, return @callback and
    // setup a Composable BackHandler to handle the action.
  ...
  override fun onRestoreInstanceState(savedInstanceState: Bundle) {
    ...
    if (savedInstanceState != null) {
      // Experience States
      val experienceStateOrdinal = savedInstanceState.getInt(MainActivityStateKeys.experienceState)
